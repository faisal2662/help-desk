...
class Chat extends Controller
{
	...
	public function notifikasi (Request $request){
		...
		->join('tb_kontak','tb_log_kontak.id_kontak','=','tb_kontak.id_kontak')
		->join('tb_pegawai','tb_log_kontak.id_pegawai','=','tb_pegawai.id_pegawai')
		->where([['tb_log_kontak.id_pegawai', Session::get('id_pegawai')],['tb_log_kontak.delete_log_kontak','N'],['tb_kontak.delete_kontak','N'],['tb_log_kontak.status_log_kontak','Delivery']])
		->get();

		return number_format($log_kontak->count());
	}
	...
	public function kirim_chat (Request $request){
		...
		if($cek_pegawai->count() < 1){
			...
			if($cek_kontak->count() < 1){
				...
				$values = array(
				  'role_kontak' => $data_cek_pegawai->employee_name .' ('.$data_cek_pegawai->sebagai_posisi.')',
	...
	public function suara_chat (Request $request){
	 ...
	 public function cekPesan(Request $request)
    {
        $time = Carbon::now()->subSeconds(4)->toDateTimeString();

        $pesan = ChatModel::with('employee.NamaPosisi')->where('room_chat', $request->id_kontak)->where('tgl_chat', '>', $time)->orderBy('tgl_chat', 'desc')->get();
        foreach ($pesan as $key => $value) {
            # code...

            $value['time'] = tes(date('Y-m-d H:i:s', strtotime($value->tgl_chat)));
        }

...
$status_pengaduan = ['Semua', 'Pending', 'Checked', 'Approve', 'Read', 'Solved',  'Friend', 'Holding', 'Moving', 'On Progress', 'Late', 'Finish'];
$data_pegawai = App\Models\Pegawai::with('NamaPosisi')
        ...
        if ($data_session_pegawai->sebagai_pegawai == 'Administrator') {
            ...
            $pengaduan = DB::table('tb_pengaduan')
                ...
                ->where(function ($query) {
                    $query->where('tb_pengaduan.status_pengaduan', '=', 'Approve')->orWhere('tb_pengaduan.status_pengaduan', '=', 'On Progress')->orWhere('tb_pengaduan.status_pengaduan', '=', 'Read')->orWhere('tb_pengaduan.status_pengaduan', '=', 'Moving')->orWhere('tb_pengaduan.status_pengaduan', '=', 'Finish');
                })
                ->orderBy('tb_pengaduan.id_pengaduan', 'DESC')
                ->paginate(12);
            ...
            if ($unit_kerja->count() > 0) {   
                if ($data_status_pengaduan == 'Semua') {
                    ...
                    if ($data_unit_kerja->id_bagian_kantor_pusat != 0) {
                        $bagian = DB::table('tb_kantor_pusat')
                        ...
                        $bagian = DB::table('tb_kantor_cabang')
                        ...
                        $bagian = DB::table('tb_kantor_wilayah')
                            ->where('tb_kantor_wilayah.id_kantor_wilayah', $data_unit_kerja->id_bagian_kantor_wilayah)
                            ->where('tb_kantor_wilayah.delete_bagian_kantor_wilayah', 'N')
                            ->first();
                ...
                if ($kepala_unit->id_bagian_kantor_pusat != 0) {
                    ...
                    $pengaduan = DB::table('tb_pengaduan')
                        ...
                        ->where(function ($query) {
                            $query->where('status_pengaduan', '=', 'Approve')->orWhere('status_pengaduan', '=', 'On Progress')->orWhere('status_pengaduan', 'Read')->orWhere('status_pengaduan', 'Solved')->orWhere('tb_pengaduan.status_pengaduan', '=', 'Finish');
                        })
                        ...
                        @else
                            <li class="nav-item {{ Request()->filter == $menu ? 'active' : '' }}">
                                <a class="nav-link"
                                    ...
                                    <span class="menu-title">{{ $rm->menu }}</span>
                                    {{-- @php
                                        dd(is_null($menu_notifikasi));
                                    @endphp --}}
                                    {{-- @if (is_null($menu_notifikasi)) --}}
                                        @if ($menu_notifikasi[$menu] > 0)
                                            &nbsp;&nbsp;&nbsp;
                                            <span class="badge badge-danger" style="border: 2px solid #ffffff;">
                                                <?= number_format($menu_notifikasi[$menu]) ?>

...
if ($data_pegawai->sebagai_posisi == 'Kepala Unit Kerja') {
    ...
    if ($kantor_unit) {
        ...
        $pengaduan = App\Models\Pengaduan::where('id_bagian_kantor_pusat', $kantor_unit->id_bagian_kantor_pusat)
            ->where('id_bagian_kantor_cabang', $kantor_unit->id_bagian_kantor_cabang)
            ->where('id_bagian_kantor_wilayah', $kantor_unit->id_bagian_kantor_wilayah)
            ->where('delete_pengaduan', 'N')
            ->get();

        foreach ($pengaduan as $value) {
            $resultKontak= DB::table('tb_kontak')
                ->join('tb_pegawai', 'tb_kontak.created_pengaduan', '=', 'tb_pegawai.id_pegawai')
                ->where([['tb_kontak.id_pengaduan',  $value->id_pengaduan], ['tb_kontak.delete_kontak', 'N']])
                ->orderBy('tb_kontak.tgl_kontak', 'desc')
                ->get();

    // Tambahkan hasil kontak dari setiap pengaduan ke array $allKontak
    $kontak = array_merge($kontak, $resultKontak->toArray());
            // Lakukan sesuatu dengan data $kontak jika diperlukan
        }
...
// $kontak->orderBy('tgl_kontak', 'desc');

usort($kontak, function ($a, $b) {
    return  strtotime($b->tgl_kontak) <=> strtotime($a->tgl_kontak);
});


?>

<?php if(is_null($kontak)){ ?>

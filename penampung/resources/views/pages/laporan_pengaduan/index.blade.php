...
if ($pegawai->count() < 1) {
    ...
    if ($data_pegawai->NamaPosisi->sebagai_posisi == 'Kepala Unit Kerja') {
        $status_pengaduan = ['Checked', 'Approve', 'Read', 'Moving', 'Solved', 'On Progress', 'Late', 'Finish', 'Friend'];
    } else {
        $status_pengaduan = ['Pending', 'Checked', 'Approve', 'Read', 'Moving', 'Solved', 'On Progress', 'Late', 'Finish', 'Friend'];
    }
}
...
@section('content')
    ...
    if ($session_pegawai->count() < 1) {
        ...
        if ($data_session_pegawai->id_bagian_kantor_pusat != 0) {
            $kantor = DB::table('tb_bagian_kantor_pusat')
                ->where('id_bagian_kantor_pusat', $data_session_pegawai->id_bagian_kantor_pusat)
                ->where('delete_bagian_kantor_pusat', 'N')
                ->first();
            $id_bagian = $kantor->id_bagian_kantor_pusat;
            $id_kantor = $kantor->id_kantor_pusat;
        } elseif ($data_session_pegawai->id_bagian_kantor_cabang != 0) {
            $kantor = DB::table('tb_bagian_kantor_cabang')
                ->where('id_bagian_kantor_cabang', $data_session_pegawai->id_bagian_kantor_cabang)
                ->where('delete_bagian_kantor_cabang', 'N')
                ->first();
            $id_bagian = $kantor->id_bagian_kantor_cabang;
            $id_kantor = $kantor->id_kantor_cabang;
        } elseif ($data_session_pegawai->id_bagian_kantor_wilayah != 0) {
            $kantor = DB::table('tb_bagian_kantor_wilayah')
                ->where('id_bagian_kantor_wilayah', $data_session_pegawai->id_bagian_kantor_wilayah)
                ->where('delete_bagian_kantor_wilayah', 'N')
                ->first();
            $id_bagian = $kantor->id_bagian_kantor_wilayah;
            $id_kantor = $kantor->id_kantor_wilayah;
        }
        ...
        // 	}
        // }
        elseif ($data_session_pegawai->NamaPosisi->sebagai_posisi == 'Staff' || $data_session_pegawai->NamaPosisi->sebagai_posisi == 'Staf' || $data_session_pegawai->NamaPosisi->sebagai_posisi == 'Kepala Bagian Unit Kerja') {
            if ($status == '0') {
                if ($klasifikasi == '0') {
                    $pengaduan = DB::table('tb_pengaduan')
                        ->where('tb_pengaduan.id_from_bagian', '=', $id_bagian)
                        ->where('tb_pengaduan.id_from_kantor', '=', $id_kantor)
                        ->where('tb_pengaduan.delete_pengaduan', '=', 'N')
                        ->where(function ($query) use ($tgl_awal, $tgl_selesai) {
                            $query->whereRaw('date_format(tb_pengaduan.tgl_pengaduan,\'%Y-%m-%d\') >= ? ', [date('Y-m-d', strtotime($tgl_awal))])->whereRaw('date_format(tb_pengaduan.tgl_pengaduan,\'%Y-%m-%d\') <= ? ', [date('Y-m-d', strtotime($tgl_selesai))]);
                        })
                ...
                if ($status == '0') {
                    if ($klasifikasi == '0') {
                        if ($kepala_unit->id_bagian_kantor_pusat != 0) {
                            $bagian_kantor = DB::table('tb_bagian_kantor_pusat')
                                ->where('delete_bagian_kantor_pusat', 'N')
                                ->where('id_kantor_pusat', $kepala_unit->id_bagian_kantor_pusat)
                                ->pluck('id_bagian_kantor_pusat')
                                ->toArray();
                            $pengaduan = DB::table('tb_pengaduan')
                                ->whereIn('tb_pengaduan.id_from_bagian', $bagian_kantor)
                                ->where('tb_pengaduan.delete_pengaduan', '=', 'N')
                                ->where('tb_pengaduan.status_pengaduan', '!=', 'Pending')
                                ->where(function ($query) use ($tgl_awal, $tgl_selesai) {
                                    $query->whereRaw('date_format(tb_pengaduan.tgl_pengaduan,\'%Y-%m-%d\') >= ? ', [date('Y-m-d', strtotime($tgl_awal))])->whereRaw('date_format(tb_pengaduan.tgl_pengaduan,\'%Y-%m-%d\') <= ? ', [date('Y-m-d', strtotime($tgl_selesai))]);
                            ...
                            $bagian_kantor = DB::table('tb_bagian_kantor_cabang')
                                ->where('delete_bagian_kantor_cabang', 'N')
                                ->where('id_kantor_cabang', $kepala_unit->id_bagian_kantor_cabang)
                                ->pluck('id_bagian_kantor_cabang')
                                ->toArray();
                            $pengaduan = DB::table('tb_pengaduan')
                                ->whereIn('tb_pengaduan.id_from_bagian', $bagian_kantor)
                                ->where('tb_pengaduan.delete_pengaduan', '=', 'N')
                            ...
                            $bagian_kantor = DB::table('tb_bagian_kantor_wilayah')
                                ->where('delete_bagian_kantor_wilayah', 'N')
                                ->where('id_kantor_wilayah', $kepala_unit->id_bagian_kantor_wilayah)
                                ->pluck('id_bagian_kantor_wilayah')
                                ->toArray();
                            $pengaduan = DB::table('tb_pengaduan')
                                ->whereIn('tb_pengaduan.id_from_bagian', $bagian_kantor)
                                ->where('tb_pengaduan.delete_pengaduan', '=', 'N')
                    ...
                    if ($klasifikasi == '0') {
                        if ($kepala_unit->id_bagian_kantor_pusat != 0) {
                            ...
                            $bagian_kantor = DB::table('tb_bagian_kantor_cabang')
                                ...
                                ->toArray();
                            $pengaduan = DB::table('tb_pengaduan')
                                ->whereIn('tb_pengaduan.id_bagian', $bagian_kantor)
                                ->where('tb_pengaduan.delete_pengaduan', '=', 'N')
                                ->where('tb_pengaduan.status_pengaduan', '!=', 'Pending')
                                ->where('tb_pengaduan.status_pengaduan', '=', $status)
                                ->where(function ($query) use ($tgl_awal, $tgl_selesai) {
                        ...
                        <table class="table table-bordered table-striped table-hover" style="width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= $data_pegawai->employee_name . ' (' . $data_pegawai->sebagai_posisi . ')' ?>
                                    </td>

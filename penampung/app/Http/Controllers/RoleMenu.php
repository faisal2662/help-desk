...
// 		return redirect()->route('role.menu');
	...
	function updateMenu(Request $request){
        ...
        // Update database sesuai dengan urutan baru
        // Misal, Anda memiliki model Item
        foreach ($newOrder['item'] as $index => $itemId) {
            DB::table('tb_role_menu')->where('id_role_menu',$itemId)->update(['position' => $index]);
        }

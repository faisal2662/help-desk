...
class PelangganController extends Controller
{
    ...
    function __construct()
    {
        ...
        require_once base_path('get_api.php');
    }
    ...
    public function getApi(Request $request)
    {
        ...
        // return $data;
        // try {
        //     File::put($filePath, $data_user_active);
        //     echo "Data berhasil disimpan ke $filePath ";
        // } catch (\Exception $e) {
        //     echo "Gagal menyimpan data: " . $e->getMessage();
        // }
        // $pegawai = DB::table('tb_pegawai')->pluck('id_pegawai', 'employee_id')->toArray();


        // $data = [];
        // foreach ($result as $val) {
        //     if(!array_key_exists($val['EMPLOYEE_ID'], $pegawai)){
        //         $data[]   = $val;
        //     }
        //     # code...
        // }
        ...
        // // $bagianKantorCabang = DB::table('tb_bagian_kantor_cabang')->where('delete_bagian_kantor_cabang', 'N')->pluck('id_bagian_kantor_cabang', 'nama_bagian_kantor_cabang')->toArray();
        // $kantorWilayah = DB::table('tb_kantor_wilayah')->where('delete_kantor_wilayah', 'N')->pluck('id_kantor_wilayah', 'nama_kantor_wilaya', 'N')->toArray();
        // $kantoPusat = DB::table('tb_kantor_pusat')->where('delete_kantor_pusat','N')->pluck('id_kantor_pusat', 'nama_kantor_wilayah')->toArray();
        ...
        //     $data_pegawai = DB::table('tb_pegawai')->where('employee_id', $pegawai['employee_id'])->first();
        //     if (!is_null($data_pegawai)) {
        //         $pegawai['created_by'] = 'api-sunfish';
        //         $pegawai['created_date'] = date('Y-m-d H:i:s');
        //         $data_pegawai->update($pegawai);
        //     } else {
        ...
        //             $pegawai['id_position_pegawai'] = $resultPosisiMapping[$posisiPegawai];
        //         } else {
        //             $pegawai['id_Position_pegawai'] = null;
        //         }
        //         $kepala = 'Kepala ' . $pegawai['DIVISION_NAME'];
        //         if($pegawai['BRANCH_NAME'] == 'Kantor Pusat'){

        //                 if(array_key_exists($pegawai['DIVISION_NAME'], $bagianKantorPusat)){
        //                     $pegawai['id_bagian_kantor_pusat'] = $bagianKantorPusat[$pegawai['DIVISION_NAME']];
        //                    $get_pegawai = DB::table('tb_pegawai')->insertGetId($pegawai);

        //                     if($pegawai['POSITION_NAME'] == $kepala){
        //                         $kepala_unit = DB::table('tb_kepala_unit')->insert([
        //                             'id_pegawai' => $get_pegawai['id_pegawai'],
        //                             'id_bagian_kantor_pusat'=> $pegawai['id_bagian_kantor_pusat'],
        //                             'id_bagian_kantor_cabang' => 0,
        //                             'id_bagian_kantor_wilayah'=> 0,
        //                         ]);
        //                     }
        //                 }else if(array_key_exists($pegawai['DEPARTMENT_NAME'], $kantorCabang)){
        ...
        //                        $kepala_unit = DB::table('tb_kepala_unit')->insert([
        //                            'id_pegawai' => $get_pegawai['id_pegawai'],
        //                            'id_bagian_kantor_cabang'=> $pegawai['id_bagian_kantor_cabang'],
        ...
        //         } else if(array_key_exists($pegawai['BRANCH_NAME'], $kantorCabang)){
        ...
        //                ]);
        //            }
        //         }else if(array_key_exists($pegawai['BRANCH_NAME'], $kantorWilayah)){
        //             $idKantorWilayah = $kantorWilayah[$pegawai['BRANCH_NAME']];
        //             $bagianKantorWilayah = DB::table('tb_bagian_kantor_Wilayah')->where('delete_bagian_kantor_wilayah','N')->where('id_kantor_wilayah',$idKantorWilayah)->pluck('id_bagian_kantor_wilayah', 'name_bagian_kantor_wilayah')->toArray();
        //             $sectionName = preg_replace('/KW .*/', '', $pegawai['section_name']);
        //             $sectionName = trim($sectionName);
        //             $sectionName = 'Bagian ' . $sectionName;
        ...
        //                 $kepala_unit = DB::table('tb_kepala_unit')->insert([
        //                     'id_pegawai' => $get_pegawai['id_pegawai'],
        //                     'id_bagian_kantor_wilayah'=> $pegawai['id_bagian_kantor_wilayah'],
        //                     'id_bagian_kantor_cabang' => 0,
        //                     'id_bagian_kantor_pusat'=> 0,
        //                 ]);
        //             }
        ...
        //67 for ($i = 0; $i < count($request->id_menu); $i++) {
        ...
        // }
        //* chat gpt *//
        // $filePath = base_path('../data/data_full_new.json');
        ...
        //         $namaKantorDuplikat = DB::table('tb_bagian_kantor_cabang')
        //     ->where('delete_bagian_kantor_cabang', 'N')
        //     ->select('nama_bagian_kantor_cabang')
        //     ->groupBy('nama_bagian_kantor_cabang')
        //     ->havingRaw('COUNT(*) > 1')
        //     ->pluck('nama_bagian_kantor_cabang');

        // $bagianKantorCabang = DB::table('tb_bagian_kantor_cabang')
        //     ->where('delete_bagian_kantor_cabang', 'N')
        //     ->whereIn('nama_bagian_kantor_cabang', $namaKantorDuplikat)
        //     ->get();

        //             return $bagianKantorCabang;

        // foreach ($bagianKantorPusat as $key => $value) {
        //     // Menggabungkan nama bagian dengan nama kantor pusat
        //     // Misalkan kita hanya ingin menggabungkan dengan nama kantor pusat pertama
        //     $firstKantorPusat = reset($kantorPusat); // Mengambil nilai pertama dari kantor pusat
        //     $bagian_kantor_pusat = $key . ' ' . $firstKantorPusat; // Menggabungkan dengan spasi

        //     // Simpan hasil yang telah diproses
        //     $processedBagianKantorPusat[$bagian_kantor_pusat] = $value;
        // }
        ...
        //                 // Simpan hasil yang telah diproses
        //                 $processedBagianKantorPusat[ $bagian_kantor_pusat] = $value;
        //         // $processedBagianKantorPusat[strtolower(str_replace(' ', '', $bagian_kantor_pusat))] = $value;
        ...
        try {
            ...
            // Mengembalikan hasil yang telah diproses




            //     $kantorCabang = DB::table('tb_kantor_cabang')
            //     ->where('delete_kantor_cabang', 'N')
            //     ->pluck('id_kantor_cabang', 'nama_kantor_cabang')
            //     ->toArray();
            ...
            // $processedBagianKantorCabang = []; // Array untuk menyimpan hasil yang diproses

            // foreach ($bagianKantorCabang as $bagian) {
            //     // Jika nama kantor cabang di bagian cocok dengan key dari $kantorCabang
            //     if (isset($kantorCabang[$bagian->nama_bagian_kantor_cabang])) {
            //         $processedBagianKantorCabang[] = [
            //             'id_bagian' => $bagian->id_bagian,
            //             'id_kantor' => $kantorCabang[$bagian->nama_bagian_kantor_cabang],
            //         ];
            //     }
            // }
            //  return $es = $bagianKantorPusat[$pegawai['DIVISION_NAME']];


            //     return $processedBagianKantorCabang;
            // return $processedBagianKantorPusat;
            // Membuat array kunci dari posisi yang telah diproses
            $resultPosisiMapping = [];
            foreach ($posisiMapping as $key => $value) {
    ...
    public function datatables(Request $request)
    {
     ...
     public function lastSync(){
        $date = DB::table('tb_pegawai')->orderBy('updated_date')->first('updated_date');
        $date = Carbon::parse($date->created_date)->translatedFormat('l, d F Y h:i');
    ...
    public function edit($id)
    {
        ...
        return view('pages.user.ubah', compact('pegawai', 'nama_posisi'));
    }

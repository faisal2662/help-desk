...
use App\Http\Controllers\API\FirebaseMessagingController;
use PDO;
use DateTime;
...
class PengaduanController extends Controller
{
    ...
    public function Transaksi($filter)
    {
        $session_pegawai = \App\Models\Pegawai::with('NamaPosisi')
            ...
            if ($data_session_pegawai->sebagai_pegawai == 'Administrator') {
                if ($filter == 'Semua') {
                    ...
                    $pengaduan = DB::table('tb_pengaduan')
                        ...
                        ->where('tb_pengaduan.status_pengaduan', '=', $filter)
                ...
                if ($unit_kerja->count() < 1) {
                    if ($filter == 'Semua') {
                        $pengaduan = DB::table('tb_pengaduan')
                            ->whereRaw(
                                ' tb_pengaduan.id_pegawai IN (
                                 Select tb_pegawai.id_pegawai  From tb_pegawai
                                   Where tb_pegawai.kantor_pegawai = "' .
                                    $data_session_pegawai->kantor_pegawai .
                                    '" And  tb_pegawai.id_bagian_kantor_pusat = "' .
                                    $data_session_pegawai->id_bagian_kantor_pusat .
                                    '" And tb_pegawai.id_bagian_kantor_cabang = "' .
                                    $data_session_pegawai->id_bagian_kantor_cabang .
                                    '" And  tb_pegawai.id_bagian_kantor_wilayah = "' .
                        ...
                        $pengaduan = DB::table('tb_pengaduan')
                            ->whereRaw(
                                ' tb_pengaduan.id_pegawai IN ( Select  tb_pegawai.id_pegawai From tb_pegawai
                                            Where
                    ...
                    if ($filter == 'Semua') {
                        $pengaduan = DB::table('tb_pengaduan')
                            ->whereRaw(
                                ' tb_pengaduan.id_pegawai IN (  Select
    ...
    public function data_grid(Request $request)
    {
        ...
        if ($session_pegawai->count() > 0) {
            ...
            foreach ($pengaduan as $data_pengaduan) {
                ...
                $klasifikasi = 'klasifikasi_data(' . $data_pengaduan->id_pengaduan . ", '" . $data_pengaduan->nama_pengaduan . "')";

                # code...
            }
        }
        return view('pages.pengaduan.data_grid', compact('pengaduan', 'data_session_pegawai', 'pegawai', 'kantor_pegawai', 'bagian_pegawai', 'bagian_pengaduan', 'kantor_pengaduan', 'status_klasifikasi', 'lampiran', 'jawaban', 'status_pengaduan', 'delete', 'approve', 'checked', 'finish', 'klasifikasi'))->render();
        ...
        if ($session_pegawai->count() < 1) {
            ...
            if ($data_session_pegawai->sebagai_pegawai == 'Administrator') {
                ...
                if ($unit_kerja->count() < 1) {
                    ...
                    $pengaduan = DB::table('tb_pengaduan')
                        ->whereIn('tb_pengaduan.id_pegawai', function ($query) use ($data_session_pegawai) {
                            $query
                                ->select('tb_pegawai.id_pegawai')
                                ->from('tb_pegawai')
                                ->whereIn('tb_pegawai.kantor_pegawai', function ($subquery) use ($data_session_pegawai) {
                                    $subquery
                                        ->select('kantor_pegawai')
                                        ->from('tb_kepala_unit_kerja')
                                        ->where('delete_kepala_unit_kerja', 'N')
                                        ->where('id_pegawai', $data_session_pegawai->id_pegawai);
                                })
                                ->whereIn('tb_pegawai.id_bagian_kantor_pusat', function ($subquery) use ($data_session_pegawai) {
                                    $subquery
                                ...
                                ->whereIn('tb_pegawai.id_bagian_kantor_cabang', function ($subquery) use ($data_session_pegawai) {
                                    $subquery
                                ...
                                ->whereIn('tb_pegawai.id_bagian_kantor_wilayah', function ($subquery) use ($data_session_pegawai) {
                                    $subquery
        ...
        if (!is_null($data_pengaduan)) {
            ...
            if ($data_pengaduan->status_pengaduan == 'Approve') {
                ...
                DB::table('tb_notifikasi')->insert([
                    'id_pegawai' => $data_pengaduan->id_pegawai,
                    'nama_notifikasi' => 'Pengaduan Read',
                    'keterangan_notifikasi' => 'Pengaduan "' . $data_pengaduan->nama_pengaduan . '" sudah dibaca.',
                ...
                DB::table('tb_dibaca')->insert(['id_pengaduan' => $id, 'id_pegawai' => auth()->user()->id_pegawai, 'tgl_dibaca' => date('Y-m-d H:i:s'), 'created_by' => auth()->user()->employee_name]);
            }
        }
...
function time_elapsed_string($datetime, $full = false)
    ...
    public function datatables(Request $request)
    {
        ...
        if ($session_pegawai->count() < 1) {
            ...
            foreach ($pengaduan as $data) {
				...
				';
                $data->dari_pengaduan =  $kantor_pegawai . ' - ' . $bagian_pegawai;
    ...
    public function save(Request $request)
    {
        ...
        if (!empty($data_pengaduan)) {
            ...
            //     $namaKantor = $bagian_kantor->headOffice->nama_kantor_cabang;
            //     $namaBagian = $bagian_kantor->nama_bagian_kantor_cabang;
            // } else {
            //     $bagian_kantor = BagianKantorWilayah::with('regionalOffice')->where('id_bagian_kantor_wilayah', $data_pengaduan->id_bagian_kantor_wilayah)->where('delete_bagian_kantor_wilayah', 'N')->first();
            //     $namaKantor = $bagian_kantor->headOffice->nama_kantor_wilayah;
                    ...
                    if ($get_pegawai->count() > 0) {
                        ...
                        foreach ($get_pegawai as $data_get_pegawai) {
                            ...
                            if ($data_pengaduan->id_bagian_kantor_pusat != 0) {
                                ...
                                $namaKantor = $bagian_kantor_cabang->headOffice->nama_kantor_cabang;
                                ...
                                $namaKantor = $bagian_kantor_wilayah->headOffice->nama_kantor_wilayah;
                            ...
                            foreach ($pegawai as $data_pegawai) {
                                ...
                                $kode_pengaduan =  'P' . date('y') . '-0000' . $data_pengaduan->id_pengaduan;
                                $subject = 'Pengaduan Baru' . $kode_pengaduan;
                                $name = $data_list_pegawai->employee_name;
                                $mail = new PHPMailer(true);                              // Passing true enables exceptions
                                try {
                                    // Pengaturan Server
                                    //    $mail->SMTPDebug = 2;                                 // Enable verbose debug output
    ...
    public function cek_habis_sla(Request $request)
    {
        ...
        $pengaduan = Pengaduan::with('Pegawai')
            ->where('delete_pengaduan', 'N')
            ->where('status_pengaduan', '!=', 'Finish')
            ->whereIn('status_pengaduan', ['On Progress', 'Approve', 'Read'])
            ->get();
        $smtp = DB::table('tb_smtp')->first();
        $sla_kurang_sehari = '';
        $pegawai = [];
        ...
        foreach ($pengaduan as $value) {
            ...
            $sla_kurang_sehari = Carbon::parse($value->sla_pengaduan)->subDay(1)->toDateString();
            $sla_kurang_lima = Carbon::parse($value->sla_pengaduan)->subDay(5)->toDateString();

            if ($date == $sla_kurang_sehari) {
                $id_pengaduan[] = $value->id_pengaduan ;
                if ($value->id_bagian_kantor_pusat) {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_pusat', $value->id_bagian_kantor_pusat)->select('tb_customer.employee_name', 'tb_customer.email')->get();
                } else if ($value->id_bagian_kantor_cabang) {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_cabang', $value->id_bagian_kantor_cabang)->select('tb_customer.employee_name', 'tb_customer.email')->get();
                } else {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_wilayah', $value->id_bagian_kantor_wilayah)->select('tb_customer.employee_name', 'tb_customer.email')->get();
                }
                dd($pegawai);
            ...
            if ($date == $sla_kurang_lima) {
                $id_pengaduan[] = $value->id_pengaduan ;
                if ($value->id_bagian_kantor_pusat) {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_pusat', $value->id_bagian_kantor_pusat)->select( 'tb_pegawai.email')->get();
                } else if ($value->id_bagian_kantor_cabang) {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_cabang', $value->id_bagian_kantor_cabang)->select('tb_pegawai.email')->get();
                } else {
                    $pegawai  = DB::table('tb_pegawai')->where('id_bagian_kantor_wilayah', $value->id_bagian_kantor_wilayah)->select( 'tb_pegawa.email')->get();
                }
        ...
        // return response()->json(['id_pengaduan' => $id_pengaduan], 200);
    }
        ...
        if ($pengaduan->count() < 1) {
            ...
            foreach ($pengaduan as $data_pengaduan) {
                ...
                if (!is_null($data_pegawai)) {
                    ...
                    if ($data_pegawai->kantor_pegawai == 'Kantor Pusat') {
                        $kantorBagian = BagianKantorPusat::where('id_bagian_kantor_pusat', $data_pegawai->id_bagian_kantor_pusat)->where('delete_bagian_kantor_pusat', 'N')->first();
                        $idKantorPusat = $kantorBagian->id_kantor_pusat;
                    } else if (array_key_exists($data_pegawai->kantor_pegawai, $kantorCabang)) {
                        $kantorBagian = BagianKantorCabang::where('id_bagian_kantor_cabang', $data_pegawai->id_bagian_kantor_cabang)->where('delete_bagian_kantor_cabang', 'N')->first();
                        $idKantorCabang = $kantorBagian->id_kantor_cabang;
                    } else if (array_key_exists($data_pegawai->kantor_pegawai, $kantorWilayah)) {
                        $kantorBagian = BagianKantorWilayah::where('id_bagian_kantor_wilayah', $data_pegawai->id_bagian_kantor_wilayah)->where('delete_bagian_kantor_wilayah', 'N')->first();
                        $idKantorWilayah = $kantorBagian->id_kantor_wilayah;
                    }
                    ...
                    if ($get_pegawai->count() > 0) {
                        foreach ($get_pegawai as $data_get_pegawai) {
                            ...
                            // if($data_get_pegawai->kantor_pegawai == 'Kantor Pusat'){
                            //     $values['keterangan_notifikasi'] = 'Pengaduan baru "' . $data_pengaduan->nama_pengaduan . '" telah diajukan oleh ' . $data_pegawai->employee_name;
                            // // $get_pegawai = DB::table('tb_pegawai')
                            // //     ->where('tb_pegawai.id_pegawai', '!=', $data_pegawai->id_pegawai)
                            // //     ->where('tb_pegawai.delete_pegawai', '=', 'N')
                            // //     ->where('tb_pegawai.status_pegawai', '=', 'Aktif')
                            ...
                            // //     ->get();
                            $get_pegawai = KepalaUnit::where('delete_kepala_unit_kerja', 'N')
                                ->where('id_bagian_kantor_pusat', $idKantorPusat)
                                ->where('id_bagian_kantor_cabang', $idKantorCabang)
                                ->where('id_bagian_kantor_wilayah', $idKantorWilayah)
                                ->get();

                            if ($get_pegawai->count() > 0) {
                                $notificationEmployeeIds = [];
                                foreach ($get_pegawai as $data_get_pegawai) {
                                    $notificationEmployeeIds[] = $data_get_pegawai->id_pegawai;
                                ...
                                FirebaseMessagingController::sendMessageToManyEmployee(
                                    $notificationEmployeeIds,
                                    'Pengaduan Checked',
                                    'Pengaduan baru "' . $data_pengaduan->nama_pengaduan . '" telah diajukan oleh ' . $data_pegawai->employee_name,
                                    [
                                        'screen' => '/customer-complaint-detail',
                                        'id' => $data_pengaduan->id_pengaduan
                                    ]
                                );
                            }
    ...
    public function data_grid_friend()
    {
                        ...
                        $pengaduan = pengaduan::with('BagianKantorWilayah')
                            ...
                            ->where(function ($query) {
                                $query->where('tb_pengaduan.status_pengaduan', '=', 'Approve')
                                    ...
                                    ->orWhere('tb_pengaduan.status_pengaduan', '=', 'Finish')
                                    ->orWhere('tb_pengaduan.status_pengaduan', '=', 'Read');
                            })->orderBy('id_pengaduan', 'desc')->paginate(12);
                    }
        ...
        foreach ($pengaduan  as $data_pengaduan) {
            ...
            // get data pegawai
            $data_pegawai = DB::table('tb_pegawai')
                ->join('tb_posisi_pegawai', 'tb_pegawai.id_pegawai', '=', 'tb_posisi_pegawai.id_posisi_pegawai')
                ->where([['tb_pegawai.id_pegawai', $data_pengaduan->id_pegawai]])
                ->first();
            $kantor_pegawai = '-';
            $bagian_pegawai = '-';
        ...
        return view('pages.pengaduan.data_grid_from', compact('pengaduan', 'data_session_pegawai', 'data_pegawai',  'kantor_pegawai', 'bagian_pegawai', 'bagian_pengaduan', 'kantor_pengaduan', 'status_klasifikasi', 'lampiran', 'jawaban', 'status_pengaduan', 'klasifikasi'))->render();
    }
